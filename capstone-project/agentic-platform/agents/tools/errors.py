...
trailing fence, an empty body. A toy loop crashes on these. The platform **repairs what it
safely can**, and for the rest hands the model a precise error result so it can fix the call on
the next turn — the cheapest, most robust recovery there is.

This module is pure (no I/O, no model calls) so the repair rules are unit-testable in isolation.
Each agent variant wires it in; the policy lives here.
"""
...
class MalformedToolCall(ValueError):
    ...
    A loop catches this and turns it into a model-readable error result rather than crashing — the
    message is written to be actionable, because the model is what reads it and retries.
    """
...
def repair_arguments(raw: Any) -> dict[str, Any]:
    ...
    Handles, in order: an existing ``dict`` (returned as-is); ``None``/``""`` (→ ``{}``, a no-arg
    call is legitimate); a JSON *string* (parsed, fences stripped first); a parsed-but-non-object
    value (rejected — a tool's arguments must be a named mapping). Raises :class:`MalformedToolCall`
    with an actionable message on anything it can't repair.
    """
...
def repair_tool_call(*, id: str, name: Any, arguments: Any) -> ToolCall:
    """Build a clean :class:`ToolCall` from a model's raw fields, or raise :class:`MalformedToolCall`.

    A missing/blank tool *name* is unrecoverable here (nothing to dispatch to), so it raises; the
    loop reports it back to the model. Arguments are run through :func:`repair_arguments`.
    """
...
class RetryPolicy:
    """How many *consecutive* failing turns a loop tolerates before giving up.
    ...
    broken call. The count resets the moment a turn makes progress.
    """
