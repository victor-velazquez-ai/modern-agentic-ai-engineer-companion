...
Same :class:`~rag.stores.base.VectorStore` surface as the in-memory store, backed by a local
`Chroma <https://www.trychroma.com/>`_ collection so vectors survive process restarts and scale
past what fitting everything in a Python dict comfortably allows.
...
keeps ``import rag`` working with zero third-party deps installed — the whole point of
runnable-by-default — while still giving you real persistence when you opt in. The connection
target comes from the ``CHROMA_URL`` env var (or the ``persist_directory`` argument); secrets and
endpoints are read from the environment, never hardcoded.
"""
...
from .memory import idf_table, keyword_score
from .memory import _term_set  # reuse the lexical scorer for the keyword channel
...
class ChromaVectorStore:
    """A :class:`~rag.stores.base.VectorStore` backed by a local Chroma collection.

    Args:
        collection_name: Chroma collection to create/use.
        persist_directory: where Chroma persists data. ``None`` falls back to ``CHROMA_URL`` (if
            set) or an ephemeral in-process client (handy for tests of the adapter itself).
    """

    def __init__(
        self,
        collection_name: str = "agentic_platform",
        *,
        ...
        persist_directory = persist_directory or os.getenv("CHROMA_URL") or None
    ...
    def keyword_search(self, query: str, k: int) -> list[StoredChunk]:
        ...
        # same IDF-weighted scorer as the in-memory store, so behavior stays identical. A
        # production swap would back this with a real full-text/sparse index.
        if k <= 0 or len(self) == 0:
            return []
        ...
        texts = [t or "" for t in dump["documents"]]
        idf = idf_table(texts)

        from ..ingest.chunk import Chunk

        scored: list[StoredChunk] = []
        for cid, text, meta in zip(dump["ids"], texts, dump["metadatas"]):
            score = keyword_score(q_tokens, text, idf)
            if score <= 0.0:
                continue
            meta = meta or {}
            chunk = Chunk(
                ...
                metadata={kk: vv for kk, vv in meta.items() if kk not in {"doc_id", "index"}},
            )
