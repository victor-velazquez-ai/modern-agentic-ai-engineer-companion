"""Input / output guardrails (Ch 41) — PII, prompt-injection, content safety.

Two questions Ch 41 makes you answer before a model call ships:

* **Input guards** run on the prompt *before* it reaches the provider. They block obvious
  prompt-injection ("ignore previous instructions and …") and can redact PII so secrets never
  leave your perimeter.
* **Output guards** run on the model's text *before* it reaches the user. They redact PII the
  model may have echoed and flag unsafe content.

These are *defaults you can enforce*, not a complete safety system — the regexes here are
deliberately simple and readable. The value is the **seam**: one place every call passes
through, so the policy is consistent and testable. Swap the detectors for a real classifier /
DLP service without touching callers. This is the same guard the ``llm/gateway.py`` runs on
every model call; ``security/`` owns it so the policy lives in one reviewable place.

Guards fail **closed** for blocks (a blocked input raises) and **safe** for redaction
(unknown → leave alone), which is the conservative default Ch 41 argues for.
"""
...
_PII_PATTERNS: list[tuple[str, Pattern[str], str]] = [
    ...
    (
        "api_key",
        # Provider key prefixes (sk-, sk-ant-, pk-, ghp_, xoxb-) followed by a long token
        # that may itself contain one internal '-' or '_' separator.
