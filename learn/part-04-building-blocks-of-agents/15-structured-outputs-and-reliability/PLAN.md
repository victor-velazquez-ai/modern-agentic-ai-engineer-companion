...
- **Cell arc:**
  ...
  - Assemble `complete_structured()` (constrained → one retry → repair → metrics →
    ...
    computable question.
  - Close by pointing at `capstone/llm/structured.py` (every model call in Parts VI–VIII goes
    through it).
...
- **Capstone:** advances `capstone/llm/structured.py` — the platform-wide structured-output
  choke point that `capstone/evals/` (Ch 22) and the observability stack (Ch 23) hang off;
...
- [ ] 15-02 links `capstone/llm/structured.py`; each notebook ends with recap + 2–4 exercises;
