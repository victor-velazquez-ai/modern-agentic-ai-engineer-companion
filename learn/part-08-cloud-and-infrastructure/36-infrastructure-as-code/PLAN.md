...
`templates/terraform-module/` and `capstone/infra/`.
...
- **Cell arc:**
  ...
  - Ends pointing at the production module: [`templates/terraform-module/`](../../../templates/terraform-module/)
    and the capstone's `capstone/infra/`.
...
- **Capstone:** **produces** `capstone/infra/` — the full Terraform for the Ch 33 architecture
...
- [ ] 36-01 ends pointing at `templates/terraform-module/` and `capstone/infra/`; no secrets in
      `.tf`/state/committed outputs — referenced by ARN only.
- [ ] Each notebook ends with recap + exercises and links onward (template, `capstone/infra/`,
