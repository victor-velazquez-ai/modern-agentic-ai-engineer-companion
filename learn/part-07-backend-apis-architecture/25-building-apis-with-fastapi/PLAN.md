...
- **Cell arc:**
  ...
  - 📋 ends by pointing at `templates/fastapi-agent-service/` (the scaffold) and
    `capstone/app/` (the production version): "you built the toy; here's the real one."
...
- **Capstone:** builds `capstone/app/` (the API surface from §25.5); checkpoint
  `checkpoints/ch25-backend-api`. The queue handoff lands fully in Ch 31 (`capstone/workers/`).
