...
- **Template(s):** the worker + broker services extend
  ...
  (api + worker + Redis).
- **Capstone:** **builds `capstone/workers/`** (Celery app, `run_agent`, Beat schedules,
  idempotent task patterns) and the async `/runs` dispatch in `capstone/app/`; advances the
  checkpoint/resume integration with `capstone/` checkpointing (Ch 14). Checkpoint
