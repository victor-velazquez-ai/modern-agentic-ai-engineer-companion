...
- **The capstone backend gets built and hardened.** Ch 25 stands up the API surface
  (`capstone/app/`), Ch 26 makes it enterprise-grade (auth, limits, webhooks), Ch 28 shapes its
  internals (hexagonal, DI, multi-tenancy), Ch 30 gives it a real data layer, and Ch 31 puts the
  slow work behind durable workers (`capstone/workers/`). Most chapters end by pointing at
  [`templates/fastapi-agent-service/`](../../templates/fastapi-agent-service/) — the production
  scaffold — and the matching `capstone/` checkpoint: *build the toy here, study/lift the real one.*
...
| 31 | Distributed Backends & Automation | Walkthrough — Celery async runs + schedules, n8n automations → `capstone/workers/`. *(sibling-authored)* | [`31-distributed-backends-and-automation/PLAN.md`](31-distributed-backends-and-automation/PLAN.md) |
...
first capstone backend milestone (`capstone/app/`, checkpoint `ch25-backend-api`). If you only
