...
- **Capstone:** establishes `capstone/db/` (schema, Alembic migrations, SQLAlchemy models),
  `capstone/cache/` (Redis cache-aside + LLM cache), and the `pgvector` storage under
  `capstone/rag/`; checkpoint `checkpoints/ch30-data-layer`.
