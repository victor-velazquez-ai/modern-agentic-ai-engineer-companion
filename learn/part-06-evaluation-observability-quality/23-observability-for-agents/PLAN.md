...
- **Capstone:** advances `capstone/telemetry.py` (the instrumented agent loop) and wires the
  *instrument*/*observe* stations into `capstone/` that gate and explain every run; checkpoint
...
- [ ] Each notebook ends with recap + 2–3 change-and-predict exercises and a link to
      `blueprints/observability-stack/` and `capstone/telemetry.py`.
