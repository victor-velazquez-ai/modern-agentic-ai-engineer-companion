...
`capstone/llm/gateway.py` are born.
...
- **Cell arc:**
  ...
  - Close by pointing at `blueprints/llm-gateway/` (the production version: real providers,
    telemetry hooks, rate limits) and `capstone/llm/gateway.py`.
...
- **Capstone:** **builds `capstone/llm/gateway.py`** — the platform's single model chokepoint
...
- [ ] 39-01 ends pointing at `blueprints/llm-gateway/` + `capstone/llm/gateway.py`; each notebook
