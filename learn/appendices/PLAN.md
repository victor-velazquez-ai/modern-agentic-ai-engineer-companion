...
`capstone/` catalogs). This map is the hand-off: it tells those catalogs which appendix grounds
...
- **Becomes:** a split between **quick-reference notebooks** (where running beats reading) and
  **`templates/` snippets** (where the value is copy-paste scaffolding). Card-by-card:
  ...
  | Celery (31) | the config block seeds [`../../capstone/`](../../capstone/) `workers/celery_app.py` and the Ch 31 walkthrough | capstone + notebook |
...
- **Becomes:** [`../../capstone/`](../../capstone/) — the companion folder that *realizes* this
...
- **Owner:** [`../../capstone/`](../../capstone/) plan (this map only flags C as its blueprint).
...
- **Becomes:** two assets:
  ...
  2. The **capstone's readiness pass** — [`../../capstone/`](../../capstone/) carries a filled-in
...
- **Owner:** a `templates/` plan (the blank) + the `capstone/` plan (the worked pass); this map
...
- **Template(s):** `production-readiness-checklist` (from F); FastAPI/Docker cards (from B) →
  [`../../templates/`](../../templates/).
- **Capstone:** the whole [`../../capstone/`](../../capstone/) tree (C is its canonical layout);
...
## Dependencies
- The `blueprints/`, `templates/`, and `capstone/` catalog plans consume this map (esp. the G slug
...
- [ ] `capstone/` tree matches Appendix C exactly; `.env.example` covers C's var table (C).
