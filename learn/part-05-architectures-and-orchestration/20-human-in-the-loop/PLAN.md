...
- **Capstone:** builds `capstone/approvals.py` + the gate in `capstone/loop.py` (park as
...
- [ ] Recap + 2–4 exercises per notebook; secrets from env only; links resolve to
      `blueprints/agent-loop/`, `blueprints/multi-agent-supervisor/`, and `capstone/` approvals.
