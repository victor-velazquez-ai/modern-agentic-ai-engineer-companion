...
- **Cell arc:**
  ...
  - 🎯 senior lens: tune the recall weights to the domain (support → recency; medical →
    importance). Close by pointing at `blueprints/memory-module/` and `capstone-project/memory/`.
...
- **Capstone:** advances `capstone-project/memory/` (the `Memory` class + ranked recall + skill
  library) and contributes the checkpoint scheme used by `capstone-project/agents/` and
  `capstone-project/workers/` (Ch 31); checkpoint `checkpoints/ch14-memory`.
...
- [ ] 14-02 links `blueprints/memory-module/` + `capstone-project/memory/`; the skill library is gated
