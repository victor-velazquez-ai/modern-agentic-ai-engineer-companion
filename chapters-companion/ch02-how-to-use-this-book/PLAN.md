...
- **Map the book's structure onto the repo.** A short section in
  ...
  `templates/` scaffolds · `capstone-project/` answer key). States plainly: **you still build the
  capstone yourself; `capstone-project/` is to check/compare/unblock, not to clone.**
...
- **Capstone:** — (orientation only; reinforces the "build the capstone yourself, then compare
  against `capstone-project/`" framing the whole repo depends on).
