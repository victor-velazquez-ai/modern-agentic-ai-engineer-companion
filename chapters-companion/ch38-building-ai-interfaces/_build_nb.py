...
These knobs live in [`../../capstone-project/web/`](../../capstone-project/web/) (the throttle/memo config), not here. If your real chat stutters while streaming, this — not the network — is almost always why.
"""))
...
- **Capstone (the finished thing):** [`../../capstone-project/web/`](../../capstone-project/web/) — the complete reference frontend wired to the FastAPI backend: streaming chat, `RunTimeline`, `ToolCard`, `ApprovalCard`, the BFF stream proxy, and `iron-session` auth. Milestone: `checkpoints/ch38-web-frontend`. The browser talks **only** to the Next.js origin; FastAPI ([`../../capstone-project/app/`](../../capstone-project/app/), Ch 25/26) stays on a private network.
