...
+ `capstone-project/web/`** application, *not* a notebook. What a notebook *can* teach, and teach well, is
...
- **Maps to:** book §38.1 (streaming UIs / Vercel AI SDK — *protocol only*), §38.2 (rendering
  ...
  🔧 **Build (the Next.js frontend) is a `templates/web-starter/` + `capstone-project/web/` app, not a
...
- **Cell arc:**
  ...
  - ⚠️ pitfall (perf, *named not coded*): the naive append-token-then-re-render-everything churn —
    ...
    Explain why (React-specific), and point to `capstone-project/web/` where the throttle/memo knobs live.
...
- **You'll be able to:** see any streaming agent UI as a fold over a typed event stream, validate
  ...
  views — then build the real surface in `templates/web-starter/` + `capstone-project/web/`.
...
- **`capstone-project/web/`** — the complete reference frontend wired to the capstone backend (chat,
...
- **Capstone:** advances [`capstone-project/web/`](../../capstone-project/web/) — the platform's user-facing
  ...
  (`capstone-project/app/`, Ch 25/26) stays on a private network.
...
- Ch 37 (TS/React/Next mental model + discriminated unions) · Ch 25 (SSE/WebSocket streaming
  ...
  Requires `capstone-project/app/` (the FastAPI backend) to exist as the thing the frontend proxies.
...
- [ ] The notebook is explicit that **§38.5's Build is the `templates/web-starter/` + `capstone-project/web/`
      app, not a notebook**, and ends by linking both.
- [ ] `templates/web-starter/` and `capstone-project/web/` PLAN/scaffold own the actual UI; the
