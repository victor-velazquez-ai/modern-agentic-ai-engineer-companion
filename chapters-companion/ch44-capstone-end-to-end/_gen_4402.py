...
This is a **worksheet**: prompts, fill-in cells, and a few *runnable gates* where a check can actually fire. It runs **fully offline** in `MOCK=1` — the rollout/shadow simulations and the checklist are all local; the live gates ride on `capstone-project/`'s own CI and `docker compose` smoke test (opt-in, ⚠️ flagged). *Build yours first from the 🔧 Build sections, then use this pass to check it.*
""")
...
**Packages:** Python standard library only (`os`, `random`, `statistics`). Offline and free in `MOCK=1`. The live gates (real eval suite, real cap) run on `capstone-project/`'s CI and `docker compose` — opt-in, ⚠️ flagged.
""")
...
The book's master operational artifact — seven sections, each line compressing a chapter. Run it before any launch, the capstone's or your own. Mark each box **owned / checked / flagged** against `capstone-project/` (or your system), and keep the chapter reference so you can jump back to the detail.
...
# after walking each box against capstone-project/ (or your system).
SECTIONS = [
...
- **Run it for real (opt-in, ⚠️):** the live gates ride on the capstone's own CI and smoke test. From the repo root: `cd capstone && docker compose up` brings up the stack, and the eval suite in [`../../capstone-project/evals/`](../../capstone-project/evals/) plus the gateway caps enforce these checks against real traffic. This is opt-in because it spends real tokens.
- **Diff against known-good:** compare your readiness pass to the reference [`../../capstone-project/checkpoints/`](../../capstone-project/checkpoints/) — *build yours first, then compare.*
