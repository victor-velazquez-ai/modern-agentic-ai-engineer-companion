...
core promise); it is a guided **walkthrough that tours the finished `capstone-project/`** and a
...
tour to compare, check, and unblock.* The notebooks point hard at `capstone-project/` and its
...
- **Type:** walkthrough  *(a tour of the assembled `capstone-project/`, not a from-scratch build)*
...
- **Objective:** follow one real request ("summarize last week's feedback and file the top
  three issues") through `capstone-project/` — frontend → thin FastAPI intake → queue → Celery worker
  → agent plan/retrieve/tool-via-MCP-with-approval/checkpoint → gateway → SSE stream → trace —
  and name which `capstone-project/` directory and which book chapter owns each hop.
- **Prereqs:** the whole book; ideally your own capstone attempt + Ch 12–41 notebooks. This
  notebook *reads and traces* the reference `capstone-project/`; it assumes it's been built (or uses
  `checkpoints/`).
- **Cell arc:**
  ...
  - Map cell: render the §44.1 layer table (Agent core → Knowledge → Orchestration → Quality →
    Backend → Architecture → Data → Async → Cloud → Frontend → LLMOps) onto the `capstone-project/`
  ...
  - Closes by pointing at `capstone-project/README.md` (Appendix C mapping) and the matching
...
- **Datasets/fixtures:** tiny canned trace + 2–3 in-memory "feedback" docs so the tour runs
  with no services; the *real* request path lives in `capstone-project/`, which this notebook references.
- **APIs & cost:** mockable — `MOCK=1` replays a canned end-to-end trace (free, deterministic);
  `MOCK=0` (opt-in, ⚠️ flagged) drives the actual local `capstone-project/` via `docker compose`.
- **You'll be able to:** narrate the entire capstone as one request across four planes, and
  locate any layer in both the book and the `capstone-project/` tree.
...
- **Objective:** take `capstone-project/` (or your own system) through the book's master checklist —
...
- **Cell arc:**
  ...
  - 📋 the **full production-readiness checklist** (§44.5) as the closing artifact: walk all
    ...
    against `capstone-project/` (or your system) with the chapter reference.
...
- **Datasets/fixtures:** tiny canned eval set + a replayed traffic slice for the canary/shadow
  simulations; the real gates live in `capstone-project/evals/` and the gateway, which this references.
- **APIs & cost:** mockable — simulations and checklist run offline in `MOCK=1`; the live gates
  ride on `capstone-project/`'s own CI/`docker compose` smoke test (opt-in, ⚠️ flagged).
...
- **Capstone:** **the whole `capstone-project/`** — this chapter is its guided tour. 44-01 walks
  `capstone-project/{app,workers,agents,rag,memory,mcp,evals,infra,web}/`; 44-02 runs the readiness pass
  against `capstone-project/evals/` + the gateway + guardrails, and exercises the
  [`capstone-project/checkpoints/`](../../capstone-project/checkpoints/) scheme ("diff yours against
  known-good") and the AWS deploy (`capstone-project/infra/`, Ch 33/36).
...
- [ ] Both notebooks run top-to-bottom in `MOCK=1` (canned trace / replayed traffic) with no
      errors; the live (`MOCK=0`) path drives `capstone-project/` via `docker compose` and is opt-in.
- [ ] 44-01's layer→directory→chapter map matches §44.1's table and the actual `capstone-project/` tree.
...
- [ ] Both notebooks frame the capstone as **"build yours first, then compare"** and end by
      pointing at `capstone-project/` + the matching `checkpoints/`; all cross-links resolve.
