...
**The promise:** by the end you can follow one real request — *"summarize last week's customer feedback and file the top three issues"* — through the **whole** `capstone-project/`, naming at every hop which **plane** it touches, which `capstone-project/` directory owns it, and which chapter built it.
...
- Map each layer of §44.1's table onto a `capstone-project/` directory **and** the chapters that built it.
...
**Prereqs:** effectively the whole book — especially Ch 3 (four planes), Ch 12–20 (agent core, MCP, approval gates), Ch 22–23 (evals + observability), Ch 38–41 (frontend, gateway, security), Ch 42 (thin intake). Ideally your own capstone attempt and the Ch 12–41 notebooks. This notebook *reads and traces* the reference `capstone-project/`; it assumes it is built (or uses `checkpoints/`).

**Packages:** Python standard library only (`json`, `os`, `pathlib`). No external dependency, no API key, no network in `MOCK=1`. The live path (`MOCK=0`) drives the real local `capstone-project/` via `docker compose` and is **opt-in** and ⚠️ flagged.
""")
...
# capstone-project/ via `docker compose up` (opt-in, costs real tokens) — see the ⚠️ cell
...
The fastest way to see all of it at once is the §44.1 map: each **layer**, the `capstone-project/` **directory** that realizes it, and the **chapters** that built it. Nothing here is decorative — remove a layer and a requirement from Chapter 42's method goes unmet.
""")

code(r'''
# The §44.1 layer table, joined to the capstone-project/ directory tree and the chapters
...
w1, w2, w3 = 14, 16, 30
print(f"{'layer':<{w1}}{'plane':<{w2}}{'capstone-project/ dir':<{w3}}built in")
print("-" * 78)
...
We now follow one request through the assembled system. In `MOCK=1` we replay a canned trace from `data/canned_trace.json` — the same shape an OpenTelemetry export would produce (Chapter 23), with a `plane`, the owning `capstone-project/` directory, the chapters, latency, tokens, and cost per span.
""")
...
- The §44.1 **layer → directory → chapter map** lets you locate any concern in both the book and the `capstone-project/` tree.
...
3. **Find the layer.** Pick three chapters you have not opened in a while (say Ch 30, Ch 39, Ch 41). For each, name the **layer**, the **plane**, and the `capstone-project/` **directory** from the map — without scrolling up. Then check.
...
Everything above ran on a canned trace. To watch a **real** request flow through the live system, you would harden the loop into the actual `capstone-project/`:
...
- **The capstone itself:** read [`../../capstone-project/README.md`](../../capstone-project/README.md) (the Appendix C mapping) with this trace open, and diff your own assembly against the matching [`../../capstone-project/checkpoints/`](../../capstone-project/checkpoints/) — *build yours first, then compare.*
