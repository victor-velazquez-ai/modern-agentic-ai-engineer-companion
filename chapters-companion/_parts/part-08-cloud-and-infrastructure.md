...
| 36 | Infrastructure as Code & Platform Engineering | 🔧 Walkthrough — author a Terraform module and run `plan` locally (no `apply`), feeding `templates/terraform-module/` + `capstone-project/infra/`; plus a concept lab modeling CI/CD as a quality gate with an **eval gate**. | [`36-infrastructure-as-code/PLAN.md`](36-infrastructure-as-code/PLAN.md) |
...
`capstone-project/infra/` (Ch 36) — are what Part X (Production LLMOps) operates on.
