...
In ``MOCK=1`` (the default) we use :class:`MockSummarizer`: a deterministic, offline, extractive
summarizer that costs nothing and makes tests reproducible. In production you would inject a
summarizer backed by the ``llm-gateway`` blueprint (see ``../../llm-gateway/``) — the contract is
just the :class:`Summarizer` protocol, so the swap is one line at construction time.
"""
...
class Summarizer(Protocol):
    ...
    testable. A live implementation would call a model through ``llm-gateway``; the mock below is
    the offline default.
    """
...
def default_summarizer() -> Summarizer:
    ...
    ``COMPANION_MOCK`` defaults to ``"1"`` (mock). A live path would, when ``MOCK=0``, construct a
    gateway-backed summarizer here. We keep the standalone module honest by only shipping the mock
    and documenting the seam rather than importing a not-yet-present dependency.
    """
    mock = os.getenv("COMPANION_MOCK", "1") != "0"
    if mock:
        return MockSummarizer()
    # Live seam: ``from llm_gateway import GatewaySummarizer`` would go here once the
    # llm-gateway blueprint is built. Until then we fail loud rather than silently spend.
    raise RuntimeError(
        "COMPANION_MOCK=0 requested a live summarizer, but no llm-gateway client is wired in. "
        "Inject a Summarizer explicitly (see README §Live path) or run with COMPANION_MOCK=1."
    )
