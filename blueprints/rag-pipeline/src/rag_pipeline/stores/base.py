...
**adapter** decision. The PLAN is explicit that this must be real, tested code, not prose — so
every store implements this one tiny interface and the retriever depends only on it. Swapping
local <-> cloud is then a one-line construction change, not a rewrite.

The keyword side of hybrid search also lives behind this Protocol (:meth:`keyword_search`), so a
store backed by a real search engine (e.g. a Chroma full-text index or a BM25 service) can serve
it natively instead of the in-memory default's term-overlap scorer.
"""
...
class StoredChunk:
    ...
    ``score`` is interpreted per channel — cosine similarity for :meth:`VectorStore.search`,
    a keyword-overlap score for :meth:`VectorStore.keyword_search`. Fusion (see
    :func:`rag_pipeline.retrieve.reciprocal_rank_fusion`) consumes *ranks*, not raw scores, so
    the two channels never need a shared scale.
    """
