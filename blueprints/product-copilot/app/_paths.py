...
A solution blueprint **composes** the pattern blueprints; it does not copy them. Each pattern
blueprint ships its code under its own ``src/`` (e.g. ``../agent-loop/src/agent_loop``). This
module puts those ``src/`` directories on ``sys.path`` so this package can simply::

    from agent_loop import AgentLoop, ToolRegistry, tool
    from rag_pipeline import HybridRetriever, InMemoryVectorStore
    from llm_gateway import Gateway, Guard
    from eval_harness import run, Case, Contains
...
# This file lives at  product-copilot/app/_paths.py
...
_PATTERN_BLUEPRINTS = (
    "agent-loop",            # the in-app agent that answers and acts as the authenticated user
    "rag-pipeline",          # retrieval over product docs + the user's own *scoped* data
    "llm-gateway",           # tiered routing + fallback + caching + per-user limits (margin)
    "eval-harness",          # evals + online feedback as a first-class product metric
    "observability-stack",   # latency, cost-per-user, and abuse signals on the public surface
)
