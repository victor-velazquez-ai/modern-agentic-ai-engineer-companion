...
borrow the gateway. We **import**, we never copy: a bug fixed in ``llm-gateway`` is a bug fixed
...
Layout this relies on::
    ...
    blueprints/
      ...
      product-copilot/app/_compose.py            <- you are here

``add_pattern_blueprints_to_path()`` resolves ``blueprints/`` as ``parents[2]`` of this file
and inserts the five ``src/`` dirs (idempotently). Call it once, at the top of any module that
imports a pattern package; every module under :mod:`app` does exactly that via ``from . import
_compose``.

This solution is the **customer-facing product copilot** (Appendix G), so it composes one
pattern the internal blueprints don't lean on as hard: the ``llm-gateway`` — because on a
public surface *latency, caching, per-user limits, and abuse guards are the product*, not an
afterthought (PLAN.md → "Composes").
"""
...
# blueprints/product-copilot/app/_compose.py
#   parents[0] = app/  parents[1] = product-copilot/  parents[2] = blueprints/
