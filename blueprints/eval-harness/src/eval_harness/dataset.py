...
A golden set is a list of :class:`Case` rows stored as **JSONL** — one JSON object per
line. JSONL is the right shape for an eval set: it is appendable (add a case the day you
find a bug), diff-able, and reviewable in a pull request.
...
* ``input`` — what the candidate is given. Required (any JSON type).
...
* ``tags`` — at least one. Tags are first-class so scores can be sliced by segment
  (capability, difficulty, ``must-refuse``, a regression-ticket id) instead of collapsing
...
def parse_case(obj: dict[str, Any]) -> Case:
    ...
    Raises :class:`DatasetError` with a precise message on any missing/invalid field, so a
    bad dataset fails loudly in CI rather than silently scoring nothing.
    """
...
def load_jsonl(path: str | Path) -> list[Case]:
    ...
    Blank lines and ``#`` comment lines are skipped so a dataset can be lightly annotated.
    A duplicate ``id`` is an error — silently overwriting cases is how a golden set rots.
    """
