"""Token → cost accounting, and the per-run cost roll-up.
...
1. **Price one model call.** Given a model id and its input/output token counts, return the
   USD cost. Prices live in a table you can swap; the table is the *policy*, the math is
   the *mechanism*. (In production the ``llm-gateway`` metering layer is the source of
   truth for usage; this module is the source of truth for turning that usage into money
   and attributing it to spans.)

2. **Roll the cost up the tree.** A run's total cost is the sum of its model-call costs,
   but you also want each intermediate span to carry the cost of *its* subtree — so a
   dashboard can answer "what did the retrieval phase cost?" not just "what did the run
   cost?". ``roll_up_cost`` walks the span tree bottom-up and writes both the per-span cost
   (``agent.cost.usd`` on LLM spans) and the subtree roll-up (``agent.cost.rollup_usd`` on
   every span), returning the run total.

Prices are USD per **million** tokens and are approximate, illustrative defaults — update
them from the provider's current pricing page before trusting a real bill. The point of the
blueprint is the *shape* of the calculation, not a frozen price sheet.
"""
...
# Illustrative defaults (USD / 1M tokens). Anthropic-first, with a couple of common others
# so routing/eval examples have something to price. NOT a live price feed — see module doc.
PRICES: dict[str, ModelPrice] = {
    # Anthropic (the book's primary stack)
    "claude-opus-4": ModelPrice(15.0, 75.0),
    "claude-sonnet-4": ModelPrice(3.0, 15.0),
    "claude-haiku-4": ModelPrice(0.80, 4.0),
    ...
    # A free local/mock model so MOCK demos price to exactly $0.
...
def price_for(model: str) -> ModelPrice:
    ...
    if base in PRICES:
        return PRICES[base]
    # strip a trailing -YYYYMMDD snapshot date, e.g. claude-sonnet-4-20250514
    trimmed = base
    parts = base.rsplit("-", 1)
