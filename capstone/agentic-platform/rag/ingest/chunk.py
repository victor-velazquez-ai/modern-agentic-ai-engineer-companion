"""Chunking — the decision that bounds everything downstream (Ch 13, "Chunking").

A retriever can only return what ingestion preserved. The senior judgment lives in the chunking
strategy, not the loader: chunks must be **small enough to be specific** yet **large enough to be
self-contained**, and **overlap** so a fact split across a boundary survives in at least one
chunk. This module keeps that decision in one tested place.

Two strategies ship here:

* :func:`chunk_text` — a sliding window over **words** with a configurable size and overlap.
  Word-based (not character-based) keeps tokens roughly proportional and avoids cutting words in
  half; the book's rule of thumb (~200-400 tokens, ~10-20% overlap) maps cleanly onto it.
* *structure-aware* splitting on blank-line paragraph boundaries (``structure_aware=True``),
  which respects the document's own seams before falling back to the window.
...
# Sensible defaults (book Ch 13 rule of thumb). Word counts, not tokens, but close enough for a
# window: ~120 words is roughly ~160 tokens of English prose.
    ...
    if structure_aware:
        ...
        for para in _PARAGRAPH_SPLIT.split(text):
            ...
            if len(_words(para)) <= chunk_size:
                ...
                chunks.extend(chunk_text(para, chunk_size=chunk_size, overlap=overlap))
        return chunks
...
def chunk_documents(
    ...
    *,
    ...
    Each chunk's id is ``{doc_id}::{index}`` so re-ingesting the same corpus is idempotent (a
    chunk keeps its identity), and document metadata is copied onto every chunk for citation and
