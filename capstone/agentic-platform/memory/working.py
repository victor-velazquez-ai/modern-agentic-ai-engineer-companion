...
:mod:`memory.summarize`) so their information survives in a smaller footprint.
...
class WorkingMemory:
    ...
    summarizer:
        ...
        :class:`~memory.summarize.MockSummarizer` (no API spend).
    keep_last:
    ...
    def render(self) -> list[Message]:
        ...
        if self.rolling_summary:
            out.append(
                Message("system", f"Summary of earlier conversation: {self.rolling_summary}")
            )
    ...
    def snapshot(self) -> dict[str, Any]:
        """Serialize the window for the persistence backend / a checkpoint."""
        return {
    ...
    def restore(self, snapshot: dict[str, Any]) -> None:
        ...
        self.summary_budget_ratio = snapshot.get(
            "summary_budget_ratio", self.summary_budget_ratio
        )
