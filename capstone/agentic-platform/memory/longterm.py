...
:meth:`LongTermMemory.search`'s scorer for cosine similarity over embeddings (the platform ``rag``
subsystem / a vector DB) in production and nothing above changes. (Indeed the platform can back
long-term recall with ``rag`` once a corpus of memories is worth embedding — the seam is identical.)
"""
...
# A tiny stopword list so high-frequency function words don't manufacture false relevance (e.g. an
# unrelated query sharing only "the"/"in" should score ~0). A production embedding retriever
# handles this implicitly; for the lexical scorer we strip the worst offenders.
...
def _relevance(query_tokens: set[str], text: str) -> float:
    ...
    Cheap, deterministic, no model. The production swap is cosine similarity over embeddings; this
    keeps the retrieval contract identical while staying offline.
    """
...
class LongTermMemory:
    ...
    persistence backend (see :mod:`memory.backends`), which snapshots/restores these records. The
    store itself is backend-agnostic.
    """
    ...
    def search(
        ...
        *,
        ...
        candidates = (
            self.records if kind is None else [r for r in self.records if r.kind == kind]
        )
        scored = [(record, _relevance(query_tokens, record.text)) for record in candidates]
