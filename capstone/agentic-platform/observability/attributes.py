"""Span attribute conventions for agent-run traces (Ch 23).

One agent run becomes a tree of spans; the *value* of that tree is only as good as the
attributes we hang on each span. Conventions matter more than cleverness: if every subsystem
that produces a span (``agents/``, ``rag/``, the ``llm/`` gateway) names the run id, model,
token counts, and cost the *same* way, then a dashboard, a cost roll-up, or a CI assertion can
read any trace without special-casing the producer.

We borrow the shape of OpenTelemetry's semantic conventions (dotted, namespaced keys) but keep
the keys here as plain constants so the package has **zero import-time dependency on OTel** —
the console exporter and the cost roll-up work on stdlib alone, and the OTel-backed exporters
(see ``exporters.py``) translate these same keys when present.

Keep keys stable: Ch 23, downstream dashboards, and the tests all read them by name.
"""
...
TENANT_ID = "agent.tenant.id"  # the unit cost is attributed to (multi-tenant accounting)
...
def usage_attributes(
    *,
    ...
    This is the one place that knows how to name token usage, so every instrumented model call
    (the gateway, a raw SDK call, a test) agrees. ``cost.py`` reads exactly these keys to price
    the call.
    """
