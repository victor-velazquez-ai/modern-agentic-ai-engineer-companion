"""The tracer: build a nested span tree for one agent run, on the stdlib alone (Ch 23).

Tracing is *instrumentation you wrap around other code*. This module makes that wrapping cheap
and idiomatic:

- a :class:`Tracer` owns one run and hands out spans;
- ``with tracer.span(...)`` (and the ``run`` / ``model_span`` / ``tool_span`` /
  ``retrieval_span`` helpers) opens a child of whatever span is currently active, so nesting
  follows your call stack automatically;
- ``@traced`` decorates a function so each call becomes a span without touching the body.

Why no OpenTelemetry import here? The *concepts* — a span tree, parent/child links,
attributes, timing, status — are the thing to instrument; OTel is one serialization of them.
``exporters.py`` bridges to a real OTel backend when you want one, but the tracer stays
dependency-free so the console path, the cost roll-up, and the tests run with only the stdlib
(``COMPANION_MOCK=1``). The current span is tracked with :class:`contextvars.ContextVar`, so
nesting is correct across nested ``with`` blocks, decorated calls, and ``asyncio`` tasks.
"""
...
class Span:
    ...
    A span has a name, a :class:`SpanKind`, a bag of attributes, a parent link, an ordered list
    of children, timing, and a status. The roll-up in ``cost.py`` and the rendering in
    ...
    def record_usage(
        ...
        *,
        ...
        The matching cost is *not* computed here: pricing is a policy that lives in ``cost.py``
        and is applied by ``roll_up_cost`` so the table is swappable.
        """
...
class Tracer:
    ...
    Typical use::
        ...
        with tracer.run("support-agent"):
            with tracer.model_span("answer", model="claude-...", input_tokens=..., output_tokens=...):
                ...
    ...
    *exporter*, not this.
    ...
    def run(self, name: str, *, attributes: dict[str, Any] | None = None) -> "Any":
...
# A process-wide default tracer, so a helper or a @traced function can instrument without
# threading a Tracer through every call. A real service usually owns its tracer explicitly;
# this is the ergonomic default.
...
def traced(
    ...
    *,
    ...
    ``tracer.span(...)`` context manager when you need per-call attributes (tokens, query, k);
    the decorator is for the common "just wrap this in a span" case.
    """
