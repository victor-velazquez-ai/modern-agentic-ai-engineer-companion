"""Exporters: turn a finished trace into something you can read or ship (Ch 23).

The default is the **console exporter** — pure stdlib, no collector, no network, no API spend.
It renders the span tree as an indented outline with per-span cost and the run total, which is
exactly what you want while learning or in CI: you *see* what the agent did and what it cost
without standing up infrastructure.

The other exporters target real backends (OTLP collector, Arize Phoenix, Langfuse). They are
**optional adapters**: the heavy ``opentelemetry-*`` / vendor packages are imported *lazily
inside the method*, not at module import. That keeps the whole package importable — and the
console path, the cost roll-up, the tests all runnable — with only the stdlib, even when OTel
is not installed. Choosing and swapping an exporter is a config decision (see ``setup.py``);
the instrumentation in ``tracing.py`` never changes. Sampling lives at the exporter boundary
too — export every trace while learning, sample in production.
"""
...
class ConsoleExporter:
    ...
    def render(self, trace: Trace) -> str:
        ...
        lines.append("")
        lines.append(
            f"run {trace.run_id}  -  {trace.span_count()} spans  -  ${total:.6f} total"
        )
        unknown = cost_mod.unknown_models(trace)
        if unknown:
            lines.append(
                f"  ! unpriced models (cost may be understated): {', '.join(sorted(unknown))}"
            )
        return "\n".join(lines)
    ...
    def _label(self, span: Span) -> str:
        ...
        if attrs.COST in a and a[attrs.COST]:
            ...
            attrs.COST_ROLLUP in a
            and a[attrs.COST_ROLLUP]
            and span.kind is not attrs.SpanKind.LLM
        ):
            parts.append(f"sub ${float(a[attrs.COST_ROLLUP]):.6f}")
        if span.status is SpanStatus.ERROR:
            parts.append(
                f"ERROR: {a.get(attrs.ERROR_MESSAGE, a.get(attrs.ERROR_TYPE, 'error'))}"
            )
...
class _LazyOTelExporter:
    ...
    The OTel imports happen in :meth:`export`, so constructing one of these (or importing this
    module) never requires ``opentelemetry`` to be installed. If it is missing we raise a
    clear, actionable error instead of an ImportError deep in the stack.
    """
    ...
    def export(self, trace: Trace) -> None:  # pragma: no cover - requires optional dep
        ...
        otel_tracer = provider.get_tracer("agentic-platform.observability")
